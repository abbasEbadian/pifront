  ...
  <div>
      ...
      <b-card no-body class="col-12">
         ...
         <b-card-header class="row no-gutters align-items-center">
          ...
          <div class="col-3 cent">موضوع</div>
          <div class="col-3 cent">متن</div>
      ...
      <b-card id="form" no-body class="col-12">
          ...
          <b-card-body class="py-3 wallets">
            <form @submit.prevent="submit()" enctype="multipart/form-data">
              ...
              <b-textarea maxlength='500' v-model="minitext" placeholder="خلاصه پیام" rows="8"></b-textarea><br>
  ...
  methods: {
    ...
    async submit () {
      ...
      const formdata = new FormData()
        ...
        formdata.append('title' , this.title)
        formdata.append('minitext' , this.minitext)
        formdata.append('position' , 'mainposts')
        if (document.getElementById('file').files.length > 0){
          var img = document.getElementById('file').files[0]
          formdata.append('pic', img, img.name)
        }
      await axios
        .post(`adminpanel/pages` , formdata)
        .then(response => {
          setTimeout(() => {
            this.getp()
          }, 2000)
