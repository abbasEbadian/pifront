      ...
      <b-card no-body class="col-12" style="text-align:center">
        <b-card-header style="font-family:'arial'">
          ...
          <b-card-body class="py-3 wallets">
            <h4>
              Available <br>
              <div v-if="wallets.balance" class="col-12 cent"> {{wallets.balance.available}} <div v-if="!wallets.balance.available">0</div> </div>
              <hr>
              Frozen <br>
              <div v-if="wallets.balance" class="col-12 cent">{{wallets.balance.frozen}}<div v-if="!wallets.balance.frozen">0</div></div>
            </h4>

          </b-card-body>
      </b-card>
       <div id="withdraw"></div><br><br>
      <b-card>
        ...
        <select class="form-control" v-model="chain" @change="getfee()" style="font-family:'arial'">
          ...
          <h6> مبلغ درخواستی <a type="button" v-if="!wallets.balance.available > 0" @click="amountset()" class="btn btn-dark" style="background:white;padding:3px;color:#444">  موجودی قابل انتقال : 0</a>  <a v-if="wallets.balance.available -( parseFloat(fee) + (parseFloat(fee)/2)) > 0" @click="amountset()" class="btn btn-dark" style="background:white;padding:3px;color:#444">  موجودی قابل انتقال : {{wallets.balance.available}}</a></h6>
      ...
      <div  id="deposit"></div><br>
        <b-card >
          ...
          <form @submit.prevent="submitdep" label="" style="text-align:center">
            <img v-if="address" id="qr" :src=" `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${address}`"><br>
...
export default {
  ...
  methods: {
    ...
    amountset () {
      this.amountout = this.wallets.balance.available
    },
    depamountset () {
      this.amountin = this.wallets.balance.available
    },
    goto () {
